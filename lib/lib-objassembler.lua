...
function cObjMaker:create( psSpriteFile, poSpriteData, paMap)
	...
	oInstance = cClass.createGroupInstance("cObjMaker", cObjMaker)
...
function cObjMaker:_initobjmaker( psSpriteFile, poSpriteData, paMap)
	...
	for iItem=1,#paMap do
		...
		if oSet == nil then error ("no sprite found for mapping "..sSpriteName)	end
		...
		if self.draggable then
			...
			cDraggable:addEventListener("onDrag", self)
		end
	end
end
...
function cObjMaker:onDrag(poEvent)
	...
	if poEvent.phase == "ended" then
		...
		print ("dragged", oObj.id, oObj.x, oObj.y)
	end
	return true
end
