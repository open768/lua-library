...
function Inspector:putValue(v)
  ...
  elseif sType == 'function' then
	if bExpandFunctions then
		oFnInfo = debug.getinfo(v, 'nfS')
		self:puts(sType)
		self:putTable(oFnInfo)
	else
		self:puts('<',sType,' ',self:getOrCreateCounter(v),'>')
	end
  else
    self:puts('<',sType,' ',self:getOrCreateCounter(v),'>')
  end
...
function vardump(t, depth)
  print (inspect(t,depth))
end
